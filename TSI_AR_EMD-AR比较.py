...
path = r"C:\Users\12406\Desktop\test\论文代码\Excel\江西南昌滁槎.xlsx"
#载入时间序列数据
...
fig,axes = plt.subplots(3,1,sharex=True,figsize=(8,7),dpi=200)
fig.text(0.06,0.5,'TOC(mg/L)',va='center', rotation='vertical',fontsize=16)
fig.text(0.5,0.05,'时间序列', va='center', ha='center',fontsize=16)

axes2 = axes[0]
axes2.plot(data.index[st:et],test,'k',label='真实值')
axes2.plot(data.index[st:et],prediction1, '--r',label='TSI:MSE=%.3f' % error1)
axes2.set_ylim([0, 5])
axes2.legend(loc='upper right')

axes3 = axes[1]
axes3.plot(data.index[st:et],test,'k',label='真实值')
axes3.plot(data.index[st:et],prediction2, '--g',label='AR:MSE=%.3f' % error2)
axes3.set_ylim([0, 5])
axes3.legend(loc='upper right')

axes4 = axes[2]
axes4.plot(data.index[st:et],test,'k',label='真实值')
