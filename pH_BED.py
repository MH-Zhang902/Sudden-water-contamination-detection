...
'''
path1 = r"C:\Users\12406\Desktop\test\论文代码\Excel\江西南昌滁槎pH残差序列(倒U型)事件发生概率.xlsx"
#载入时间序列数据
...
ax[0].plot(dates,X, color='black',label='残差序列')
ax[0].set_ylim([-0.55,1.65])
ax[0].set_ylabel('pH(mg/L)',fontsize=12)
#ax.set_xlabel('时间',fontsize=16)
ax[0].axhline(0, linestyle='--',color='black')

#ax.legend(loc='right')
collection1 = collections.BrokenBarHCollection(xlist, (0,1),facecolor='green')
collection2 = collections.BrokenBarHCollection(list3, (0,0.5),facecolor='red')
ax[0].add_collection(collection1)
ax[0].add_collection(collection2)
ax[0].set_title('BED方法事件判断结果',x=0.52,y=0.9,fontsize=10)
ax[0].legend(loc='upper right')

ax[1].plot(dates,X, color='black',label='残差序列')
#ax.set_ylim([-0.05,1.05])
ax[1].set_ylabel('pH(mg/L)',fontsize=12)
ax[1].set_xlabel('时间序列',fontsize=12)
ax[1].set_ylim([-0.55,1.65])
ax[1].axhline(0, linestyle='--',color='black')
collection3 = collections.BrokenBarHCollection(xlist, (0,1),facecolor='green')
ax[1].add_collection(collection3)
collection4 = collections.BrokenBarHCollection(list6, (0,0.5),facecolor='red')
ax[1].add_collection(collection4)
ax[1].set_title('贝叶斯方法事件判断结果',x=0.52,y=0.9,fontsize=10)
